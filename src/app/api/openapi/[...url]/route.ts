...
// codegen:hash {input: 79a9a04cb35aa17703770dbc31dcab7f, output: 7e1cd77b87db84714576d28070d2bdbe, timestamp: 2025-08-06T16:32:51.234Z}
...
export const getClientCode: import('eslint-plugin-mmkal').CodegenPreset = ({dependencies: {fs}, context, meta}) => {
  ...
  const {Project, ModuleKind} = require('ts-morph') as typeof import('ts-morph')
  ...
  project.createSourceFile('client.ts', fs.readFileSync(path.join(dirname, '../../../../openapi/client.ts'), 'utf8'))
  const sourceFile = project.getSourceFileOrThrow('client.ts')
  const emitOutput = sourceFile.getEmitOutput()

  const files = emitOutput.getOutputFiles().map(file => {
    return {name: path.basename(file.getFilePath()), content: file.getText()}
  })
  const newCode = [
    `// generated at ${new Date().toISOString()}`,
    `export const clientFiles: FileEntry[] = ${JSON.stringify(files, null, 2).replace(/ "(\w+)": /, ' $1: ')}`,
  ].join('\n')
  ...
  if (normalise(newCode) === normalise(meta.existingContent)) {
    return meta.existingContent
  }
...
// generated at 2025-05-22T00:42:57.917Z
