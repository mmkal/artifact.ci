    ...
    onBeforeGenerateToken: async (pathname, payload) => {
      ...
      const insertedUploadRequest = await client.maybeOne(
        sql<queries.UploadRequest>`
          ...
          insert into upload_requests (repo_id, ref, sha, actions_run_id, actions_run_attempt)
          select
            repo.id,
            ${context.ref},
            ${context.sha},
          ...
          where (
            ...
            where
              ...
              and actions_run_attempt = ${context.runAttempt}
              and repo.html_url = ${repoData.html_url}
              and ${context.runAttempt} < 10
          ) < 20
...
export declare namespace queries {
  ...
  /** - query: `with repo as ( insert into repos (owner,... [truncated] ... < 10 ) < 20 returning upload_requests.*` */
