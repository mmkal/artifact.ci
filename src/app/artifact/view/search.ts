...
export const searchRepos = async (params: Partial<PathParams>, {offset = 0, limit = 100} = {}) => {
  const session = await auth()
  if (!session?.user.github_login) return Object.assign([], {code: 'not_logged_in'} as const)
  const owner = params.owner || session?.user.github_login

  const repos = await client.any(sql<queries.Repo>`
    select
      repos.owner,
      repos.name as repo,
      github_installations.github_id as installation_github_id,
      (select count(1) from artifacts where repo_id = repos.id) as artifact_count
    from repos
    join github_installations on github_installations.id = repos.installation_id
    where
      owner = ${owner}
      and repos.name = ${params.repo || sql`repos.name`}
    group by repos.id, github_installations.github_id
    limit ${limit} offset ${offset}
  `)
  if (!repos.length) return []

  const octokit = await getInstallationOctokit(repos[0].installation_github_id)
  ...
  for (const repo of repos) {
    ...
    if (!canAccess.canAccess) {
      logger.warn({canAccess}, 'searchRepos: checkCanAccess failed')
      return []
    }
  }
