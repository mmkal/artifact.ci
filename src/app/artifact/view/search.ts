...
export const searchArtifacts = async (params: Partial<PathParams>) => {
  const session = await auth()
  if (!session?.user.github_login) return []
  const owner = params.owner || session?.user.github_login || null
  if (!owner) return []
  const artifacts = await client.any(sql<queries.Artifact>`
    ...
    where
      owner = ${params.owner || session?.user.github_login || null}
  ...
  const repos = Object.values(Object.fromEntries(artifacts.map(({repo, owner}) => [`${owner}/${repo}`, {repo, owner}])))
...
export declare namespace queries {
  ...
  /** - query: `select a.id artifact_id, a.name, ai.type... [truncated] ...e = $3 and ai.value = $4 and a.name = $5` */
