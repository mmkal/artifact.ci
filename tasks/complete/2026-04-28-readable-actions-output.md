...
Status summary: done. The action outputs now use readable ESM entry files for repository-owned code and minified bundled vendor chunks for dependencies. Verification passed for generation, lint, syntax checks, and local action-entry smoke checks; remaining lint output is existing repo warnings.
...
- [x] Update the action build configuration or build script to produce more readable checked-in action JS. _Added `scripts/build-action.js`, which emits readable `bundled-action/{badge,upload}.js` and minified `bundled-action/vendor/**` chunks._
...
- 2026-04-28: Final shape keeps the reviewable action/domain code in `bundled-action/upload.js` and `bundled-action/badge.js`; third-party code is in minified `bundled-action/vendor/` files and shared chunks.
