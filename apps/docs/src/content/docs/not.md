...
description: Things artifact.ci explicitly doesn't try to be.
---

## A CDN

Files within your artifacts are uploaded to a storage backend. For URLs served with an identifier using a GitHub run ID, or a commit SHA, a long-lasting cache-control header is set. For URLs served with an identifier using a branch name, a short-lasting cache-control header is set (at time of writing, five minutes).
