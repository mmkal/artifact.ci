...
const getBaseUrl = () => {
  ...
  // localhost fallback.
  return process.env.BETTER_AUTH_URL || process.env.PUBLIC_DEV_URL || process.env.AUTH_URL || 'http://localhost:3000'
}

const getTrustedOrigins = () => {
  const extras = [process.env.PUBLIC_DEV_URL, process.env.BETTER_AUTH_URL, process.env.AUTH_URL].filter(Boolean)
  return Array.from(
